...
### Is this just a wrapper for the python version of youtube-dl?

No, this is a fully standalone rewrite that requires no python installation. 
You can optionally install YoutubeDL.Python.dll (which requires python), which enables to use extractors from the python version of youtube-dl too!

### Feature foo doesnt work!

This is still experimental development version so many features are not available yet.

### How can i help?

Glad you asked, this software is in early stages of development so there are many things to do!
Here are some things you can do:

- Add an extractor to YoutubeDL
- Port one of the downloaders to YoutubeDL .NET
- Clean up code
- Look for "TODO" comments and help implement them
- Design proposals are welcome
- Implement NetStandart2.0 support

Of course every form of help is welcome!

# USING THE YOUTUBEDL.DLL LIBRARY

From a .NET program, you can embed youtube-dl like this:

```c#
using YoutubeDL;
using YoutubeDL.Models;
// if you want to use the python extractors:
...
namespace Demo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            else if (result is Playlist playlist)
            {
...
Windows users can download the .zip file containing the .exe file and place it in any location on their [PATH](https://en.wikipedia.org/wiki/PATH_%28variable%29) except for `%SYSTEMROOT%\System32` (e.g. **do not** put in `C:\Windows\System32`).
...
# DESCRIPTION
**youtube-dl** is a command-line program to download videos from YouTube.com and a few more sites. It should work on your Unix box, on Windows or on macOS. It is released to the public domain, which means you can modify it, redistribute it or use it however you like.

    youtube-dl [ACTION] [OPTIONS] URL [URL...]

# OPTIONS
Keep in mind that many options dont work because this is an early alpha version.
...
If you've followed our manual installation instructions, you can simply run `youtube-dl update` (or, on Linux, `sudo youtube-dl update`).
...
From a .Net program, you can embed youtube-dl in a more powerful fashion, like this:
